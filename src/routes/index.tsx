          ...
          <section className="mx-auto max-w-7xl bg-white px-4 py-6 sm:px-6 sm:py-8">
            ...
            <ol className="divide-y divide-border">
              {articles.slice(0, 6).map((a, i) => {
                const num = i + 1;
                const isAfrica = a.category?.slug === "africa-ai";
                return (
                  <li key={a.id} className="flex items-baseline gap-3 py-2">
                    <span
                      className="text-sm font-bold tabular-nums"
                      style={{ color: isAfrica ? "var(--africa)" : "var(--brand)" }}
                    >
                      {num}
                    </span>
                    <Link
                      to="/article/$slug"
                      params={{ slug: a.slug }}
                      className="text-sm font-semibold text-foreground hover:text-[color:var(--brand)]"
                    >
                      {a.title}
                    </Link>
                  </li>
